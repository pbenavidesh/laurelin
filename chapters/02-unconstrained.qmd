...
# Unconstrained Optimization {#sec-unconstrained}
...
ggplot(eoq_df, aes(x = q, y = cost, color = component, linetype = component)) +
  ...
  scale_color_manual(
    values = c("Holding cost" = "#4C72B0", "Ordering cost" = "#DD8452",
               "Total cost"   = "#2D6A4F")
  ) +
...
```{python}
#| label: fig-eoq-curve-py
#| fig-cap: "EOQ total cost as a function of order quantity (Python)."
...
ax.plot(q_vals, holding,  label="Holding cost",  color="#4C72B0", lw=1.5)
ax.plot(q_vals, ordering, label="Ordering cost", color="#DD8452", lw=1.5)
ax.plot(q_vals, total,    label="Total cost",    color="#2D6A4F", lw=1.5)
...
::: {.callout-tip}
## Why convexity matters for SVR

In Part III we will see that the SVR optimization problem — the
objective we need to solve to fit a model — is a convex quadratic
program. Convexity guarantees that any solver that satisfies the
optimality conditions has found the global minimum. There is no risk
of a bad local solution. This is not a coincidence: the SVR formulation
is specifically constructed to preserve convexity.
:::
...
```{python}
#| label: fig-convexity-py
#| fig-cap: "Convexity: the chord between any two points on the EOQ cost curve lies above the curve (Python)."
...
```{python}
#| label: fig-gd-step-py
#| fig-cap: "One gradient descent step on the EOQ cost (Python). The arrow shows the step from $q_0$ to $q_1$ along the cost curve."
