...
ax.plot(q_vals, holding,  label="Holding cost",  color=lc("green"),  lw=1.5)
ax.plot(q_vals, ordering, label="Ordering cost", color=lc("blue"),   lw=1.5)
ax.plot(q_vals, total,    label="Total cost",    color=lc("orange"), lw=1.5)
ax.axvline(q_star, color=lc("ref_line"), linestyle="--", lw=1)
ax.text(q_star + 50, total.max() * 0.85,
        f"q* = {q_star:.0f}", fontsize=10, color=lc("ref_text"))
...
#| renderings: [light, dark]
#| echo: false
#| fig-cap: "EOQ total cost as a function of order quantity. The holding and ordering cost components are shown separately. The minimum occurs where the two components intersect."
...
# --- dark rendering ---
fig, ax = plt.subplots(figsize=(8, 5))
ax.plot(q_vals, holding,  label="Holding cost",  color=lc("green", dark=True),  lw=1.5)
ax.plot(q_vals, ordering, label="Ordering cost", color=lc("blue", dark=True),   lw=1.5)
ax.plot(q_vals, total,    label="Total cost",    color=lc("orange", dark=True), lw=1.5)
ax.axvline(q_star, color=lc("ref_line", dark=True), linestyle="--", lw=1)
ax.text(q_star + 50, total.max() * 0.85,
        f"q* = {q_star:.0f}", fontsize=10, color=lc("ref_text", dark=True))
ax.set_xlabel("Order quantity (units)", fontsize=12)
ax.set_ylabel("Annual cost ($)",        fontsize=12)
ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, _: f"${x:,.0f}"))
ax.legend(frameon=False)
...
1. The chord lies above the curve between $q_1$ and $q_2$ — this is the geometric definition of convexity.
...
ax.plot(q_vals, cost, color=lc("green"), lw=1.5, label="Total cost")
ax.plot([q1, q2], [c1, c2], color=lc("red"), lw=1.5,  # <1>
        linestyle="--", label="Chord")
ax.scatter([q1, q2], [c1, c2], color=lc("red"), s=50, zorder=5)

ax.annotate(r"$(q_1, C(q_1))$", xy=(q1, c1),
            xytext=(q1 - 100, c1 + 200), fontsize=10,
            ha="right", arrowprops=dict(arrowstyle="-", color=lc("ref_line")))
ax.annotate(r"$(q_2, C(q_2))$", xy=(q2, c2),
            xytext=(q2 + 100, c2 + 200), fontsize=10,
            ha="left",  arrowprops=dict(arrowstyle="-", color=lc("ref_line")))
...
1. The chord lies entirely above the EOQ curve between $q_1$ and $q_2$, confirming convexity.

```{python}
#| label: convexity_py_render
#| renderings: [light, dark]
#| echo: false
#| fig-cap: "Geometric interpretation of convexity. For any two points on the EOQ cost curve, the chord (dashed) lies entirely above the curve — the defining property of a convex function."
...
fig, ax = plt.subplots(figsize=(8, 5))
ax.plot(q_vals, cost, color=lc("green"), lw=1.5, label="Total cost")
ax.plot([q1, q2], [c1, c2], color=lc("red"), lw=1.5, linestyle="--", label="Chord")
ax.scatter([q1, q2], [c1, c2], color=lc("red"), s=50, zorder=5)
ax.annotate(r"$(q_1, C(q_1))$", xy=(q1, c1),
            xytext=(q1 - 100, c1 + 200), fontsize=10,
            ha="right", arrowprops=dict(arrowstyle="-", color=lc("ref_line")))
...
# --- dark rendering ---
fig, ax = plt.subplots(figsize=(8, 5))
ax.plot(q_vals, cost, color=lc("green", dark=True), lw=1.5, label="Total cost")
ax.plot([q1, q2], [c1, c2], color=lc("red", dark=True), lw=1.5, linestyle="--", label="Chord")
ax.scatter([q1, q2], [c1, c2], color=lc("red", dark=True), s=50, zorder=5)
ax.annotate(r"$(q_1, C(q_1))$", xy=(q1, c1),
            xytext=(q1 - 100, c1 + 200), fontsize=10,
            ha="right", arrowprops=dict(arrowstyle="-", color=lc("ref_line", dark=True)))
ax.annotate(r"$(q_2, C(q_2))$", xy=(q2, c2),
            xytext=(q2 + 100, c2 + 200), fontsize=10,
            ha="left",  arrowprops=dict(arrowstyle="-", color=lc("ref_line", dark=True)))
ax.set_xlabel("Order quantity (units)", fontsize=12)
ax.set_ylabel("Annual cost ($)",        fontsize=12)
ax.legend(frameon=False)
apply_dark(fig, ax)
...
3. Step size chosen to produce a visible move in the plot — not a physically optimal learning rate.
...
ax.plot(q_vals, C(q_vals), color=lc("green"), lw=1.5)

ax.annotate(
    "", xy=(q1, C(q0)), xytext=(q0, C(q0)),
    arrowprops=dict(arrowstyle="->", color=lc("red"), lw=1.5)
)                                                # <4>
ax.scatter([q0], [C(q0)], color=lc("blue"), s=60, zorder=5,
           label=r"$q_0 = 2000$")
ax.scatter([q1], [C(q1)], color=lc("red"), s=60, zorder=5,
           label=r"$q_1$")
...
2. Its derivative.
3. Illustrative step size — calibrated for visual clarity, not numerical optimality.
...
#| renderings: [light, dark]
#| echo: false
#| fig-cap: "One step of gradient descent on the EOQ cost. Starting at $q_0 = 2000$, the negative gradient points left — toward the minimum. The step lands at $q_1$."
...
ax.annotate("", xy=(q1, C(q0)), xytext=(q0, C(q0)),
            arrowprops=dict(arrowstyle="->", color=lc("red"), lw=1.5))
ax.scatter([q0], [C(q0)], color=lc("blue"), s=60, zorder=5, label=r"$q_0 = 2000$")
ax.scatter([q1], [C(q1)], color=lc("red"), s=60, zorder=5, label=r"$q_1$")
ax.set_xlabel("Order quantity (units)", fontsize=12)
ax.set_ylabel("Annual cost ($)",        fontsize=12)
ax.legend(frameon=False)
apply_light(fig, ax)
...
# --- dark rendering ---
fig, ax = plt.subplots(figsize=(8, 5))
ax.plot(q_vals, C(q_vals), color=lc("green", dark=True), lw=1.5)
ax.annotate("", xy=(q1, C(q0)), xytext=(q0, C(q0)),
            arrowprops=dict(arrowstyle="->", color=lc("red", dark=True), lw=1.5))
ax.scatter([q0], [C(q0)], color=lc("blue", dark=True), s=60, zorder=5, label=r"$q_0 = 2000$")
ax.scatter([q1], [C(q1)], color=lc("red", dark=True), s=60, zorder=5, label=r"$q_1$")
ax.set_xlabel("Order quantity (units)", fontsize=12)
ax.set_ylabel("Annual cost ($)",        fontsize=12)
ax.legend(frameon=False)
apply_dark(fig, ax)
